...
export class GestionarCitasDoctorComponent implements OnInit {
  ...
  cargarCitas(): void {
    ...
    this.isLoading = true;
    this.http
      .get<Cita[]>(`${this.URL_BASE}/citas/doctor/${doctorId}`)
      .subscribe({
        next: (citas) => {
  ...
  reprogramarCita(): void {
    if (this.reprogramarForm.valid && this.citaSeleccionada) {
      ...
      this.http
        .put(
          `${this.URL_BASE}/citas/${this.citaSeleccionada.id}/reprogramar`,
          datos
        )
        .subscribe({
          next: (response: any) => {
            this.mensaje =
              'Cita reprogramada exitosamente. El paciente ha sido notificado.';
            this.cerrarModales();
            this.cargarCitas();
            this.isLoading = false;
          },
  ...
  cancelarCita(): void {
    if (this.cancelarForm.valid && this.citaSeleccionada) {
      ...
      this.http
        .put(
          `${this.URL_BASE}/citas/${this.citaSeleccionada.id}/cancelar-doctor`,
          datos
        )
        .subscribe({
          next: (response: any) => {
            this.mensaje =
              'Cita cancelada exitosamente. El paciente ha sido notificado.';
            this.cerrarModales();
            this.cargarCitas();
            this.isLoading = false;
          },
  ...
  cargarDoctoresDisponibles(citaId: number): void {
    this.http
      .get<Doctor[]>(`${this.URL_BASE}/citas/${citaId}/doctores-disponibles`)
      .subscribe({
        next: (doctores) => {
  ...
  reasignarCita(): void {
    if (this.reasignarForm.valid && this.citaSeleccionada) {
      ...
      this.http
        .put(
          `${this.URL_BASE}/citas/${this.citaSeleccionada.id}/reasignar`,
          datos
        )
        .subscribe({
          next: (response: any) => {
  ...
  verHistorial(cita: Cita): void {
    ...
    this.http
      .get<any[]>(`${this.URL_BASE}/citas/${cita.id}/historial`)
      .subscribe({
        next: (historial) => {
  ...
  confirmarCita(cita: Cita): void {
    // Confirmar acción
    if (
      !confirm(`¿Está seguro de confirmar la cita con ${cita.pacienteNombre}?`)
    ) {
      return;
    }
    ...
    this.http.put(`${this.URL_BASE}/citas/${cita.id}/confirmar`, {}).subscribe({
      next: (response: any) => {
